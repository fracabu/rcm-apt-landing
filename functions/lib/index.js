...
exports.scrapeConcerts = exports.sendBookingNotification = void 0;
...
const smtpEmail = ((_a = functions.config().smtp) === null || _a === void 0 ? void 0 : _a.email) || 'info@romacaputmundiapt.it';
const smtpPassword = ((_b = functions.config().smtp) === null || _b === void 0 ? void 0 : _b.password) || '';
...
function createBookingEmailTemplate(data, nights) {
...
function createBookingEmailText(data, nights) {
    const bookingDate = new Date().toLocaleDateString('it-IT', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
...
function createCustomerConfirmationTemplate(data, nights) {
    const bookingDate = new Date().toLocaleDateString('it-IT', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
...
function createCustomerConfirmationText(data, nights) {
    const bookingDate = new Date().toLocaleDateString('it-IT', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
...
exports.scrapeConcerts = functions.https.onRequest(async (req, res) => {
    try {
        ...
        validConcerts.forEach(concert => {
            ...
            batch.set(docRef, Object.assign(Object.assign({}, concert), { lastUpdated: admin.firestore.Timestamp.now() }));
        });
        await batch.commit();
...
async function scrapeAuditorium() {
    const concerts = [];
    try {
...
async function scrapeSantaCecilia() {
    const concerts = [];
    try {
...
function determineStatus(status, canBuy) {
    if (!canBuy || status === 'sold-out' || status === 'esaurito')
        return 'sold-out';
    if (status === 'few-tickets' || status === 'pochi-posti')
        return 'few-tickets';
    return 'available';
}
